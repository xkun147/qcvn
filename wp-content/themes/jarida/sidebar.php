...
<?php
	...
	$tie_sidebar_pos = $tie_sidebar_post = $sidebar_narrow_bbPress = $sidebar_page = $cat_sidebar = $sidebar_archive = '';
	...
	if ( is_home() ){
		...
		if( $tie_sidebar_pos != 'full' ){
			...
			if( !empty( $get_meta["tie_sidebar_narrow_post"][0] ) )
		...
		if( !empty( $cat_options['cat_narrow_sidebar'] ) )
