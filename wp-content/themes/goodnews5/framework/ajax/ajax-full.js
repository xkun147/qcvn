...
jQuery(document).ready(function($) {
	...
	jQuery(".mom_mailchimp_subscribe").submit( function(e){
		...
		if (email === '')
		{
			sf.before('<span class="message-box error">'+momAjaxL.error+'<i class="brankic-icon-error"></i></span>');
		}
		else
		{
			jQuery.ajax({
...
jQuery(document).ready(function($) {
		...
		jQuery(".nb-footer a.show-more-ajax").click(function(e){
			...
			jQuery.ajax({
				...
				url: momAjaxL.url,
				dataType: 'html',
				data: "action=nbsm&nonce="+momAjaxL.nonce+"&display="+display+"&category="+category+"&tag="+tag+"&nbs="+nbs+"&number_of_posts="+nop+"&sort="+sort+"&orderby="+orderby+"&offset="+offset+"&offset_all="+offset_rest+"&format="+format+"&image_size="+image_size+"&excerpt_length="+excerpt_length,
				beforeSend : function () {
