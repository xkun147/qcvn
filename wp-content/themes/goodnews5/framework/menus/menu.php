...
function start_el(&$output, $item, $depth = 0, $args = Array(), $id = 0) {
    ...
    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
		...
		<div class="mom_icons_selector">
                        <a class="mom_select_icon_menu button" data-id="<?php echo $item_id; ?>"><?php _e('Add Icon','framework'); ?></a>
        ...
        if( $depth == 0 ) {
           ...
           $menu_color = '';
            ...
            //display
                ...
                if ($item->mdisplay == 'icon') {
                        $menu_icon  = ! empty( $item->micon ) ? '<i class="icon_only '.esc_attr( $item->micon ).'"></i>' : '';
                        ...
                        $menu_icon  = ! empty( $item->micon ) ? '<i class="'.esc_attr( $item->micon ).'"></i>' : '';
				...
				if($child->object == 'category'){
			        ...
			        $r = new WP_Query( apply_filters( 'widget_posts_args', 
			          ...
			          if ($r->have_posts()) :
							...
							$output.= "><div class='subcat-thumbnail'><a href='".get_permalink()."' title='".get_the_title()."'><img src='".mom_post_image($img_size)."' alt=''></a></div><div class='subcat-title'><a href='".get_permalink()."' title='".get_the_title()."'> ".get_the_title()."</a><span> ".$sep." ". human_time_diff( get_the_time('U'), current_time('timestamp') ) ." ago </span></div></li>";
					            } 
