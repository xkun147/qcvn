...
// Our stylesheets
	...
	if (mom_option('mom_color_skin') == 'black') {
		wp_enqueue_style( 'black-style', get_template_directory_uri() . '/css/black.css' );
	}
...
<?php } 
// author box
function mom_author_box ($style = '') { ?>
<?php if ($style != 'min') { ?>
<h2 class="single-title"><?php _e('About the author', 'theme'); ?></h2>
<?php } ?>
            <div class="base-box single-box about-the-author">
                <div class="author_avatar"><?php echo get_avatar( get_the_author_meta( 'user_email' ), '80' ); ?></div>
                <div class="author_desc">
                    <h3 calss="vcard author"><span class="fn"><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"><?php the_author_meta( 'display_name' ); ?></a></span> <?php if ($style == 'min') { if (mom_option('author_np') == 1) { ?><span class="articles-count"><?php _e('Articles'); ?> <?php echo number_format_i18n( get_the_author_posts() ); ?><?php } } ?> </span></h3>
        <p>
		<?php
        ...
        <ul>
                    ...
                    <?php if (get_the_author_meta('facebook') != '') { ?><li class="facebook"><a target="_blank" href="<?php the_author_meta('facebook'); ?>"><i class="fa-icon-facebook "></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('twitter') != '') { ?><li class="twitter"><a target="_blank" href="<?php the_author_meta('twitter'); ?>"><i class="fa-icon-twitter"></i></a></li><?php } ?>
                   <?php if (get_the_author_meta('googleplus') != '') { ?><li class="googleplus"><a target="_blank" href="<?php the_author_meta('googleplus'); ?>" rel="author"><i class="fa-icon-google-plus"></i></a></li><?php } ?>
                    <li class="rss"><a target="_blank" href="<?php echo get_author_feed_link(get_the_author_meta('ID')); ?>"><i class="fa-icon-rss"></i></a></li>
                    <?php if (get_the_author_meta('youtube') != '') { ?><li class="youtube"><a target="_blank" href="<?php the_author_meta('youtube'); ?>"><i class="fa-icon-youtube"></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('dribbble') != '') { ?><li class="dribbble"><a target="_blank" href="<?php the_author_meta('dribbble'); ?>"><i class="fa-icon-dribbble"></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('vimeo') != '') { ?><li class="vimeo"><a target="_blank" href="<?php the_author_meta('vimeo'); ?>"><i class="momizat-icon-vimeo"></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('pinterest') != '') { ?><li class="pinterest"><a target="_blank" href="<?php the_author_meta('pinterest'); ?>"><i class="fa-icon-pinterest"></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('instagram') != '') { ?><li class="instgram"><a target="_blank" href="<?php the_author_meta('instagram'); ?>"><i class="fa-icon-instagram"></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('tumblr') != '') { ?><li class="tumblr"><a target="_blank" href="<?php the_author_meta('tumblr'); ?>"><i class="fa-icon-tumblr"></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('linkedin') != '') { ?><li class="linkedin"><a target="_blank" href="<?php the_author_meta('linkedin'); ?>"><i class="fa-icon-linkedin"></i></a></li><?php } ?>
                    <?php if (get_the_author_meta('soundcloud') != '') { ?><li class="soundcloud"><a target="_blank" href="<?php the_author_meta('soundcloud'); ?>"><i class="momizat-icon-soundcloud"></i></a></li><?php } ?>
        </u>
...
function mom_related_posts () { ?>
	...
	<?php
		global $post;
		$relatedby = mom_option('related_posts_by');
		if ($relatedby == '') {
			$relatedby = 'category';
		}
...
function mom_apple_pointer() {
    $pointer_content = '<h3>Images Dimensions</h3>';
    $pointer_content .= '<p><strong>iPhone:</strong> Portrait: 224*384 - Landscape: 556*294<br><strong>iPad:</strong> Portrait: 318*424<br><strong>Macbook Air:</strong> side: 420*262 - full: 570*357<br><strong>Macbook Pro:</strong> side: 408*253 - full: 601*373<br><strong>iMac:</strong> side: 515*325 - full: 576*363</p>';
?>
   <script type="text/javascript">
   //<![CDATA[
   jQuery(document).ready( function($) {
    //jQuery selector to point to
$('.select_device').next().find('#mom_apple_layout_description').find('a').click(function(e) {
	e.preventDefault();
	$(this).pointer({
...
function wpse_46028_enqueue_admin_scripts() {
    ...
    //hook the pointer 
    add_action('admin_print_footer_scripts', 'mom_apple_pointer' );
}
		...
		<tr>
			...
			<td>
				<select name="ap_l" id="ap_l">
					...
					<option value="m1" <?php selected(get_the_author_meta( 'ap_l', $user->ID), 'm1'); ?>><?php _e('Medium thumbnails', 'framework'); ?></option>
					<option value="m2" <?php selected(get_the_author_meta( 'ap_l', $user->ID), 'm2'); ?>><?php _e('Medium thumbnails2', 'framework'); ?></option>
					<option value="l" <?php selected(get_the_author_meta( 'ap_l', $user->ID), 'l'); ?>><?php _e('Large thumbnails', 'framework'); ?></option>
					<option value="g" <?php selected(get_the_author_meta( 'ap_l', $user->ID), 'g'); ?>><?php _e('Grid', 'framework'); ?></option>
					<option value="t" <?php selected(get_the_author_meta( 'ap_l', $user->ID), 't'); ?>><?php _e('Timeline', 'framework'); ?></option>
				</select>
			</td>
		</tr>
  ...
  <script>
	...
	<ul class="fslides">
		<?php foreach($slides as $slide) {
			...
			$img = wp_get_attachment_image_src($imgid, 'big-wide-img');
			$img = $img[0];
...
<?php } else { ?>
                                   <?php if(mom_post_image() != false) {?>
							<div class="feature-img"><img src="<?php echo mom_post_image('full'); ?>" alt="<?php the_title(); ?>"></div>
                                   <?php } ?>
<?php }
} //end hide feature
}
