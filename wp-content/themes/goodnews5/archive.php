...
?>
            ...
            <div class="main-col">
                <div class="category-title">
                    ...
                    <a class="bc-rss" href="<?php echo esc_url(home_url()); ?>?cat=<?php echo get_query_var('cat'); ?>&feed=rss2"><i class="fa-icon-rss"></i></a>
                    <?php } ?>
                    <?php if (is_tag() && mom_option('cat_rss') == 1) { ?>
                    <a class="bc-rss" href="<?php echo esc_url(home_url()); ?>?tag=<?php echo get_query_var('tag'); ?>&feed=rss2"><i class="fa-icon-rss"></i></a>
                    <?php } ?>
                </div>
		...
		<?php } else if ( is_date() ) {
			if ( is_day() )
				echo '<h1 class="page-title">'.__( 'Archives for ', 'framework' ) . get_the_time( 'F j, Y' ).'</h1>';
			elseif ( is_month() )
				echo '<h1 class="page-title">'.__( 'Archives for ', 'framework' ) . single_month_title( ' ', false ).'</h1>';
			elseif ( is_year() )
				echo '<h1 class="page-title">'.__( 'Archives for ', 'framework' ) . get_the_time( 'Y' ).'</h1>';
		} ?>
